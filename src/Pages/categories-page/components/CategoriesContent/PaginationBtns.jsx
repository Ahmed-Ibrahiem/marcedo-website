...
const PaginationBtns = ({ sortedProducts, gridMode, setDisplayProducts }) => {
  ...
  const paginationData = useMemo(() => {
    ...
    const packagesNumber = Math.ceil(sortedProducts.length / rowsNumber);
  ...
  }, [currentPage, sortedProducts, rowsNumber]);
  ...
  useEffect(() => {
    ...
    setDisplayProducts(products);
  }, [currentPage, rowsNumber, sortedProducts]);
  ...
  return (
    <>
      <div
        className=" mt-10 flex-between flex-wrap gap-5
      ...
      >
        ...
        <div className="pagination-btns flex-center gap-2.5 ">
          ...
          <button
            type="button"
            className={`${btnStyle} ${currentPage >= Math.ceil(sortedProducts.length / rowsNumber) ? "text-gray! bg-gray-50 cursor-auto!" : ""}`}
            onClick={() => {
              if (currentPage < Math.ceil(sortedProducts.length / rowsNumber)) {
                setCurrentPage((prev) => prev + 1);
              }
            }}
          >
        ...
        {/* Products Number Per Views */}
        <div className="flex-start gap-2.5">
          <p>Products Per View</p>
          <div
            ref={dorpDownListRef}
